...
using System.Text;
using MauiIntroductie.Data.IRepositories;
...
namespace MauiIntroductie.Data.Repositories
{
    public class WerknemerRepository : IWerknemerRepository
    {
        ...
        //werknemer teruggeven waar identifier over elkaar 
        public Werknemer GetWerknemerById(int werknemerId)
        {
            foreach (Werknemer werknemer in this.Werknemers)
            {
                if (werknemer.Id == werknemerId)
                {
                    return werknemer;
                }
            }
